...
def add_contact(args, book):
    ...
    if not any([address, email, birthday]):
        response = input(
            "Maybe you want to add more details? Such as address, email, and birthday? (yes/no): ")
        if response.lower() == 'yes':
            additional_info = input(
                "Please enter additional details (e.g., email example@gmail.com birthday DD.MM.YYYY address: text up to 100 symbols): ").split()
...
def change_contact(args, book):
    ...
    if field_to_change == "phone":
        ...
        # The old phone number is the third-to-last argument
        old_phone = args[-2]
        # Debugging line
        print(f"Debug: Trying to change phone from {old_phone} to {new_value}")
...
def add_birthday(args, book):
    if len(args) < 2:
        raise IndexError(
            "Not enough arguments. Provide a name and a birthday.")
...
def delete_contact(args, book):
    ...
    confirmation = input(
        f"Are you sure you want to delete '{name}'? (yes/no): ").strip().lower()
    if confirmation == "yes":
